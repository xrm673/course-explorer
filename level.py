...
def level_required(courses_taken,subject,min_level,
number,excluded=[],included=[]):
    ...
    """
    fulfilled = level_taken(courses_taken,subject,min_level,excluded,included)
    not_fulfilled = []
    if len(fulfilled_courses) >= number:
        return fulfilled,not_fulfilled
    for course in level(subject,min_level,excluded,included):
...
def data_level(subject,major_data,course_data,college,category):
    ...
    return level(course_data,subject,min_level,excluded,included)

# helper for level_required,
def level(course_data,subject,min_level,excluded=[],included=[]):
    """
    return a list with all courses that meet the level requirement in a subject
    """
    result = []
    for course in course_data[subject]:
        if match_level(course,subject,min_level,excluded,included):
            result.append(course)
    return result

# helper for level_required
def level_taken(courses_taken,subject,min_level,excluded=[],included=[]):
    """
    return a list of courses taken that fulfilled the level requirement
    """
    result = []
    for course in courses_taken:
        if level.match_level(course,subject,min_level,excluded,included):
            result.append(course)
    return result

def data_match_level(course_code,major_data,college,subject,category):
    """
    ...
    included = major_data[college][category]["included"]
    excluded = major_data[college][category]["excluded"]
    return match_level(course_code,subject,min_level,excluded,included)

# helper for app.level, app.level_taken
# helper for majorImportance.match_level_requirement
def match_level(course_code,subject,min_level,excluded=[],included=[]):
    """
    return true if the course match the level requirement of a subject
    """
